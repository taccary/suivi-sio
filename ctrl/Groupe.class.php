        ...
        if (!is_null($num)){ //origine=clic un bouton enregistrer ou supprimer
          ...
          if (!is_null($enregistrer)){ //on a cliquÃ© un bouton envoi
            ...
            switch ($num){
                    ...
                    if (preg_match("/^[0-9]{2}/",$get["an"])){
                      ...
                      if (!isset($get["numgroupe"])){ //crÃ©ation groupe
                        if ($model->setGroupe($nom,$an,$parcours)) {
                          $texte='Promotion cr&eacute;&eacute;';
                        } else {
                          $texte='Cette promotion existe d&eacute;j&agrave;';
                        }
                      } else { //modif groupe
                        $numgroupe=$get["numgroupe"];
                        $rep=$model->modifGroupe($numgroupe,$nom,$an,$parcours);
                        $texte='Promotion modifi&eacute;';
                      }
                    }
                  }
                }
              ...
              case 5: //on vient page téléversement fichier élève
                $repfic = 'dirrw/upload/';
                $groupe=$get["groupes"];
                if ($groupe==0){
...
//                      if ($nf!="noms.csv"){
//                        $texte="ce nom de fichier ne convient pas : $nf <> noms.csv"; //modif DR
                      if (substr($nf, -3) !="csv"){
                        $texte="Le format de fichier ne convient pas ( $nf <> csv)"; //modif DR 
                      } else{
                        if(!move_uploaded_file($tmp,$repfic.$nf)){
                          $texte="impossible de copier le fichier $nf";
                        }else{
                          ...
                          $flu = fopen($repfic.$nf,"r");

                          $nb=0; //nb erreurs
                          // recherche d'un délimiteur logique dans un fichier CSV : , ou ; 
                          while ($ligne = fgets($flu, 140)) {
                            ...
                            // une fois les trois champs identifiés, enregistrement dans la base de données
                            $tmpTable = explode($SeparCSV,$ligne); 
                            $nom=$tmpTable[0];
                            $prenom=$tmpTable[1];
                            $mel=$tmpTable[2]; 
                            echo $nom." + ".$prenom." + ".$mel; // test des données = OK... erreur vers le model ?
